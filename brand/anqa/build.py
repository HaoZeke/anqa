...
The bird and painted lockup are rasters cropped from ``source/approved.jpg``.
The typeset wordmark is outlined Plus Jakarta Sans ExtraBold (vector).
"""
...
SVG = ROOT / "svg"
PNG = ROOT / "png"
FONT = ROOT / "fonts" / "PlusJakartaSans-ExtraBold.ttf"
...
def paste_centered(base: Image.Image, mark: Image.Image, *, max_frac: float = 0.78) -> Image.Image:
...
def raster_svg(src: Path, dest: Path, width: int) -> None:
    rsvg = shutil.which("rsvg-convert")
    if rsvg is None:
        raise SystemExit("rsvg-convert not found (librsvg). On macOS: brew install librsvg")
    dest.parent.mkdir(parents=True, exist_ok=True)
    subprocess.check_call([rsvg, "-w", str(width), str(src), "-o", str(dest)])
...
def compose_stacked_clean(mark: Image.Image, word: Image.Image, gap: int) -> Image.Image:
...
def main() -> None:
    ...
    word_box = bbox_alpha(alpha, WORD_SPLIT_Y, rgba.shape[0])
    full_box = bbox_alpha(alpha, 0, rgba.shape[0])

    bird, bird_origin = crop_pad(rgba, bird_box, 16)
    painted_word, _ = crop_pad(rgba, word_box, 12)
    stacked, _ = crop_pad(rgba, full_box, 16)
    head, _ = crop_pad(rgba, (520, 206, 766, 452), 8)

    mark_img = to_image(bird)
    stacked_img = to_image(stacked)
    painted_word_img = to_image(painted_word)
    ...
    raster_svg(word_svg, PNG / "anqa-wordmark.png", 1200)
    word_type = trim_image(Image.open(PNG / "anqa-wordmark.png").convert("RGBA"), pad=8)
    ...
    save_png(light.resize((512, 512), Image.Resampling.LANCZOS), "anqa-app-icon-512.png")
    save_png(light.resize((256, 256), Image.Resampling.LANCZOS), "anqa-app-icon-256.png")
    save_png(dark, "anqa-app-icon-dark-1024.png")

    fav_base = paste_centered(rounded_canvas(256, CREAM_RGB, 48), head_img, max_frac=0.86)
    ...
    wrappers = [
        ...
        ("anqa-wordmark-ornament.svg", "anqa-wordmark-ornament.png", painted_word_img.size),
