"""Session walk and ``updates.jsonl`` keep/skip (Python + optional ``groket._scan``).

:func:`find_sessions`, :func:`looks_like_session_dir`,
:func:`keep_updates_line`, and :func:`filter_updates` always return concrete
values. ``GROKET_SCAN=0`` uses the Python body even when the extension is
installed.
"""
...
def scan_forced_off() -> bool:
    """True when ``GROKET_SCAN`` selects the Python implementation."""
    return os.environ.get("GROKET_SCAN", "1").strip().lower() in _OFF


def scan_module() -> ModuleType | None:
    """Loaded ``groket._scan``, or ``None`` when missing or forced off."""
    global _mod
    if scan_forced_off():
        return None
    if _mod is None:
        try:
            from groket import _scan as loaded
        except ImportError:
            return None
        _mod = loaded
    return _mod
