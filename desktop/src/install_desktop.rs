...
pub const APP_ID: &str = "dev.indynull.anqa-hud";
/// Overlay palette only (Sway ``for_window`` float/sticky; pop-out keeps ``APP_ID``).
pub const OVERLAY_APP_ID: &str = "dev.indynull.anqa-hud.overlay";
...
pub fn sway_overlay_rules() -> String {
    format!(
        "# anqa overlay ({OVERLAY_APP_ID}). Include from sway config:\n\
         ...
         bindsym $mod+Shift+g exec anqa hud --toggle\n"
    )
}
...
pub struct Layout {
    ...
    pub home: PathBuf,
    /// Absolute path to ``anqa-hud`` (or a wrapper target).
    pub exe: PathBuf,
...
pub fn linux_desktop_entry(exe: &Path, icon_name: &str) -> String {
    ...
    format!(
        "[Desktop Entry]\n\
         ...
         Keywords=anqa;session;trace;hud;\n"
    )
}
...
pub fn macos_launcher_script(exe: &Path) -> String {
    format!(
        "#!/bin/sh\n# Generated by anqa-hud --install-desktop\nexec {} \"$@\"\n",
...
fn install_linux(layout: &Layout) -> Result<Report, Error> {
    ...
    let sway_path = layout.home.join(".config/anqa/sway-hud.conf");
    write_bytes(&sway_path, sway_overlay_rules().as_bytes(), &mut wrote)?;
    notes.push(format!(
        "Sway: wrote {} — add: include ~/.config/anqa/sway-hud.conf",
        sway_path.display()
    ));
...
pub fn write_windows_desktop_files(layout: &Layout) -> Result<Report, Error> {
    ...
    for &(edge, png) in crate::brand::desktop_icon_pngs() {
        ...
        let path = base.join(format!("anqa-hud-{edge}.png"));
        write_bytes(&path, png, &mut wrote)?;
    }

    let ico_path = base.join("anqa-hud.ico");
...
fn install_windows(layout: &Layout) -> Result<Report, Error> {
        ...
        .status()
    {
        Ok(st) if st.success() => {
            ...
            report
                .notes
                .push("Start Menu shortcut created (anqa).".into());
        }
        Ok(st) => {
...
mod tests {
    ...
    fn temp_home(label: &str) -> PathBuf {
        let n = SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        let dir = std::env::temp_dir().join(format!("anqa-hud-install-{label}-{n}"));
    ...
    fn desktop_entry_names_app_and_exec() {
        let exe = Path::new("/opt/anqa/bin/anqa-hud");
        let body = linux_desktop_entry(exe, APP_ID);
        assert!(body.contains("Name=anqa"));
        assert!(body.contains("Exec=/opt/anqa/bin/anqa-hud --show"));
    ...
    fn shell_escape_quotes_spaces() {
        let p = Path::new("/home/user/My Apps/anqa-hud");
    ...
    fn macos_launcher_execs_path() {
        let s = macos_launcher_script(Path::new("/tmp/anqa-hud"));
        assert!(s.contains("#!/bin/sh"));
        assert!(s.contains("exec /tmp/anqa-hud"));
    }
    ...
    fn windows_ps1_quotes_paths() {
        let s = windows_shortcut_ps1(
            ...
            Path::new(r"C:\tools\anqa-hud.exe"),
            Path::new(r"C:\Users\a\AppData\Local\Anqa\hud\g.ico"),
        );
        assert!(s.contains("CreateShortcut"));
        assert!(s.contains("anqa-hud.exe"));
    ...
    fn write_macos_bundle_into_temp_home() {
        ...
        let exe = home.join("bin").join("anqa-hud");
        ...
        .unwrap();
        let app = home.join("Applications").join("anqa.app");
    ...
    fn write_windows_files_into_temp_home() {
        ...
        let exe = home.join("anqa-hud.exe");
        ...
        .unwrap();
        let ico = home.join("AppData/Local/Anqa/hud/anqa-hud.ico");
        assert!(ico.is_file());
        assert!(ico.metadata().unwrap().len() > 64);
        let ps1 = home.join("AppData/Local/Anqa/hud/create-start-menu-shortcut.ps1");
        assert!(ps1.is_file());
        assert!(report.wrote.iter().any(|p| p.ends_with("anqa-hud.ico")));
        let _ = fs::remove_dir_all(&home);
    }
    ...
    fn sway_overlay_rules_float_overlay_not_popout() {
        ...
        assert!(body.contains("anqa hud --toggle"));
    }
    ...
    fn install_linux_writes_desktop_and_icons() {
        ...
        let exe = home.join("bin/anqa-hud");
        fs::create_dir_all(exe.parent().unwrap()).unwrap();
        fs::write(&exe, b"#!/bin/sh\n").unwrap();
        ...
        let sway = home.join(".config/anqa/sway-hud.conf");
        assert!(sway.is_file());
