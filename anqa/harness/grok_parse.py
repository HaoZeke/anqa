...
def resolve_tool_display_name(title: str, raw_input: ToolInput | None = None) -> str:
    ...
    :returns: Canonical tool id for :attr:`~anqa.models.TraceEvent.tool_name`.
    """
...
# Needles live in :mod:`anqa.scan` (Python twin + optional Rust leaf).
...
def _find_container_for_session(
    ...
    # Walk parents for anqa-* container dir name
...
def _model_from_run_json(session_dir: Path, run_data: dict) -> str:
    """Map this session to the model launched for its anqa-* container.
...
def _model_from_run_parent(session_dir: Path) -> str:
    ...
    for anc in [session_dir, *session_dir.parents]:
        ...
        if not is_run_dir_name(name):
            continue
        # anqa-{12hex}-{model_suffix} or anqa-{task}-{model_short}
