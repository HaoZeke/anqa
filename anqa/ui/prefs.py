"""App-global UI preferences (``~/.anqa/config.toml``).

Thin accessors over :mod:`anqa.config`. Screens and ``panel_render``
...
def hud_global_shortcut() -> str:
    ...
    Env ``ANQA_HUD_SHORTCUT`` wins when the launcher sets it from this value.
    """
