"""Fluent localization for anqa (Project Fluent).

Catalogs: ``anqa/locale/<lang>/main.ftl`` (fallback chain ends at ``en``).

Use :func:`t` for message IDs (``t("save")``) and named variables
(``t("model-filter-notify", label="x")``). Prefer :mod:`anqa.ui_text` helpers
