...
SCHEMA_TITLE = "anqa-config"
SCHEMA_ID = "https://indynull.github.io/anqa/schemas/config.schema.json"
...
class AppConfig(BaseModel):
    ...
    theme: str = Field(
        default="auto",
        description="auto (terminal, then desktop) or a catalog / ~/.anqa/themes name.",
    )
...
def config_json_schema() -> JsonObject:
    ...
    schema["description"] = (
        "Prefs file ~/.anqa/config.toml. Authoring format is TOML; "
        "this schema describes the same keys for editors and anqa config validate."
    )
...
def load_app_config(path: Path | None = None) -> AppConfig:
    """Load ``~/.anqa/config.toml`` (or *path*). Missing or invalid → defaults.
...
def validate_config_file(path: Path) -> AppConfig:
    ...
    import leftover JSON. The CLI ``anqa config validate`` uses this path.
