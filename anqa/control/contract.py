...
METHODS: tuple[MethodSpec, ...] = (
    ...
    MethodSpec(
        name="filters/list",
        role="Saved search filters (`name`, `scope`, `query`, `holes`)",
        params=(FieldSpec("scope", "catalog, timeline, or turns. Omit for every scope."),),
        result=(
            FieldSpec(
                "filters",
                "Saved filters, name-sorted within a scope.",
                json_type="array",
            ),
        ),
        extra_md=(
            "Saved filters live in `~/.anqa/filters.toml`.\n"
            "`scope` is `catalog`, `timeline`, or `turns`.\n"
            "`field:{a,b}` is a choice hole; `field:?` is a free-text hole.\n"
            "The apps collect answers, then `filters/expand` writes a\n"
            "normal query."
        ),
    ),
    MethodSpec(
        name="filters/upsert",
        role="Insert or replace a saved filter by name and scope",
        params=(
            FieldSpec("name", "Display name.", required=True),
            FieldSpec("scope", "catalog, timeline, or turns.", required=True),
            FieldSpec("query", "Query language text (holes allowed).", required=True),
        ),
        result=(
            FieldSpec(
                "filter",
                "The stored filter (`name`, `scope`, `query`, `holes`).",
                json_type="object",
            ),
        ),
    ),
    MethodSpec(
        name="filters/remove",
        role="Delete a saved filter by name and scope",
        params=(
            FieldSpec("name", "Display name.", required=True),
            FieldSpec("scope", "catalog, timeline, or turns.", required=True),
        ),
        result=(
            FieldSpec(
                "removed",
                "True when a filter was deleted.",
                json_type="boolean",
            ),
        ),
    ),
    MethodSpec(
        name="filters/expand",
        role="Fill `field:{a,b}` and `field:?` holes",
        params=(
            FieldSpec("query", "Query with holes.", required=True),
            FieldSpec(
                "answers",
                "Hole field to value.",
                required=True,
        ...
        result=(FieldSpec("query", "Query with holes replaced."),),
    ),
)
