...
def _layout(tmp_path: Path) -> tuple[Path, Path]:
    vol = tmp_path / "traces" / "anqa-run1-m"
...
def test_events_have_open_turn_caches_by_mtime(tmp_path: Path) -> None:
    ...
    from anqa.session import turn_gate as tg
...
def test_write_follow_up_and_done_clears_pending(tmp_path: Path) -> None:
    ...
    from anqa.session.turn_gate import host_requested_done, lifecycle_state
...
def test_last_turn_staged_is_running_not_done(tmp_path: Path) -> None:
    ...
    from anqa.session.turn_gate import lifecycle_state, read_staged_follow_up
...
def test_last_turn_closed_settles_even_while_fresh(tmp_path: Path) -> None:
    ...
    from anqa.session.turn_gate import lifecycle_state
...
def test_session_needs_live_timeline_fresh_updates(tmp_path: Path) -> None:
    ...
    from anqa.session.turn_gate import session_needs_live_timeline
...
def test_finalize_session_gate_writes_done(tmp_path: Path) -> None:
    ...
    from anqa.session.turn_gate import finalize_session_gate, lifecycle_state
...
def test_traces_volume_no_gate_dirs(tmp_path: Path) -> None:
    """Volume found via grandparent fallback; no .anqa-turn* → empty list."""
    from anqa.session.turn_gate import turn_gate_dirs_for_session

    vol = tmp_path / "traces" / "anqa-run-m"
    sess = vol / "%2F" / "sess-id"
    sess.mkdir(parents=True)
...
def test_gate_command_missing_and_present(tmp_path: Path) -> None:
    from anqa.session.turn_gate import _gate_command
...
def test_read_status_file_missing_and_invalid(tmp_path: Path) -> None:
    from anqa.session.turn_gate import _read_status_file
...
def test_write_status_creates_dir(tmp_path: Path) -> None:
    from anqa.session.turn_gate import _write_status
...
def test_enqueue_empty_raises(tmp_path: Path) -> None:
    from anqa.session.turn_gate import enqueue_follow_up
...
def test_traces_volume_via_glob(tmp_path: Path) -> None:
    """Volume discovered via .anqa-turn* glob at parent.parent level."""
    from anqa.session.turn_gate import traces_volume_for_session

    vol = tmp_path / "anqa-run" / "workspace"
    sess = vol / "sess-id"
    sess.mkdir(parents=True)
    # Create .anqa-turn at the grandparent (tmp_path / "anqa-run")
    (tmp_path / "anqa-run" / ".anqa-turn").mkdir()
...
def test_ensure_gate_dirs_fallback_to_volume(tmp_path: Path) -> None:
    from anqa.session.turn_gate import _ensure_gate_dirs

    vol = tmp_path / "traces" / "anqa-run" / "workspace"
    sess = vol / "sess"
    sess.mkdir(parents=True)
    dirs = _ensure_gate_dirs(sess)
    assert len(dirs) == 1
    assert dirs[0].name == ".anqa-turn"


def test_ensure_gate_dirs_creates_fallback(tmp_path: Path) -> None:
    """When no existing gate dirs, fallback creates .anqa-turn."""
    from anqa.session.turn_gate import _ensure_gate_dirs
...
def test_follow_up_already_staged(tmp_path: Path) -> None:
    from anqa.session.turn_gate import _follow_up_already_staged
...
def test_traces_volume_parent_parent_fallback(tmp_path: Path) -> None:
    """traces_volume_for_session falls back to parent.parent when no .anqa-turn found."""
    vol = tmp_path / "traces" / "anqa-r"
    ws = vol / "workspace"
    sd = ws / "019f-sess"
    sd.mkdir(parents=True)
    # No .anqa-turn dir → falls back to parent.parent (anqa-r)
...
def test_ensure_gate_dirs_nested_volume(tmp_path: Path) -> None:
    """_ensure_gate_dirs creates .anqa-turn for nested volume layout."""
    vol = tmp_path / "traces" / "anqa-r"
    ws = vol / "workspace"
    sd = ws / "019f-sess"
    sd.mkdir(parents=True)
    # No .anqa-turn dirs exist yet
...
def test_pending_label_turn_in_progress_queued(tmp_path: Path) -> None:
    ...
    vol = tmp_path / "traces" / "anqa-run-x"
...
def test_turn_gate_dirs_base_none(tmp_path: Path) -> None:
    ...
    with patch("anqa.session.turn_gate.traces_volume_for_session", return_value=None):
...
def test_write_done_clears_pending(tmp_path: Path) -> None:
    ...
    _write_status(gate, state="awaiting_follow_up")
    enqueue_follow_up(sess, "pending")
    with patch("anqa.session.turn_gate._write_queue", side_effect=OSError("fail")):
        write_done_for_session(sess)
    assert _gate_command(gate) == "done"
...
def test_traces_volume_fallback_parent_parent(tmp_path: Path) -> None:
    ...
    sess.mkdir(parents=True)
    result = traces_volume_for_session(sess)
    # parent.parent (anqa-run) is a dir → returned as fallback
    assert result == vol
