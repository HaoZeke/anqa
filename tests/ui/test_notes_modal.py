"""Operator notes modal tests."""

from __future__ import annotations

import pytest
from groket.notes.models import NoteEntry
from groket.notes.schema import default_schema
...
@pytest.mark.asyncio
async def test_notes_modal_delete_existing() -> None:
    app = _NoteApp()
    result_holder: list[object] = []
    existing = NoteEntry.new(
        turn_index=0,
        fields={"summary": "old"},
        note_id="n-del",
    )
    ...
    async with app.run_test(size=(100, 40)) as pilot:
        ...
        app.screen.query_one("#delete-note").press()
        await wait_until(
            pilot,
            lambda: len(result_holder) == 1,
            description="delete callback",
        )
        action, nid = result_holder[0]  # type: ignore[misc]
