...
@pytest.mark.asyncio
async def test_select_all_after_unfilter_clears_filter_selection(tmp_path: Path) -> None:
    """Filter, S, clear filter, S drops that selection instead of the whole catalog."""
    app, _, _ = _make_app(tmp_path, n_sessions=4, model_ids=["alpha", "beta"])
    async with app.run_test(size=(120, 40)) as pilot:
        await wait_until(pilot, lambda: len(app._meta_only) >= 4, description="sessions loaded")
        table = app.query_one("#session-table", DataTable)
        ...
        assert len(app._selected) == 2
        app._set_session_query("")
        await wait_until(pilot, lambda: table.row_count == 4, description="filter cleared")
        assert len(app._selected) == 2
        app.action_select_all()
        await pilot.pause()
        assert app._selected == set()
