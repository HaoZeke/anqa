...
HUD uses icedtea **0.4.0** from crates.io (`icedtea = "0.4.0"`). Overlay,
hotkey, `iced::daemon`, and Textual `config.json` → tokens stay groket-owned.

## Library surfaces in use

- Session picker: `list_view` + `RowFace::Card` + `RowHeights::PerRow`
...
- Layout chrome: full-width browse (`command_bar`, `themed_scroll` for
  Overview / Findings / Notes), `status_page`, `busy_overlay`, toasts
...
- Path-pin a local icedtea checkout on this branch
