...
/// Default chord (Go to Folder family).
...
/// Human label for the default (UI chrome).
...
/// Resolve override from env / ``~/.groket/config.json``, else platform default.
///
/// Env: ``GROKET_HUD_SHORTCUT`` (wins).
/// Config: ``hud.global_shortcut`` or top-level ``hud_global_shortcut``.
///
/// Format: ``+``-separated modifiers and one key, e.g. ``Cmd+Shift+G``,
/// ``Control+Shift+Space``. Case-insensitive.
pub fn resolve_summon_shortcut() -> (Shortcut, String) {
    match load_override_string() {
        Some(raw) => match parse_shortcut(&raw) {
            ...
            Err(err) => {
                ...
                (default_shortcut(), default_shortcut_label().to_string())
            }
        },
        None => (default_shortcut(), default_shortcut_label().to_string()),
    }
}
...
/// Parse ``Mod+Mod+Key`` into a :class:`Shortcut` and a short UI label.
pub fn parse_shortcut(raw: &str) -> Result<(Shortcut, String), String> {
    ...
    Ok((Shortcut::new(Some(mods), code), label))
}
...
mod tests {
    ...
    fn parse_cmd_shift_g() {
        let (sc, label) = parse_shortcut("Cmd+Shift+G").unwrap();
        let _ = sc;
        assert!(label.contains('G') || label.contains('g') || label.contains("G"));
    }
